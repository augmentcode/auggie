 ...
 */
const KEYISH_PATTERN = /^(\.git|.*\.pem|.*\.key|.*\.pfx|.*\.p12|.*\.jks|.*\.keystore|.*\.pkcs12|.*\.crt|.*\.cer|id_rsa|id_ed25519|id_ecdsa|id_dsa)$/;
/**
 ...
 */
export function alwaysIgnorePath(path) {
    return path.includes("..");
}
/**
 * Check if a path matches the keyish pattern (secrets/keys)
 */
export function isKeyishPath(path) {
 ...
 */
export function isValidFileSize(sizeBytes, maxFileSize = DEFAULT_MAX_FILE_SIZE) {
    return sizeBytes <= maxFileSize;
}
/**
 * Check if file content is valid UTF-8 (not binary)
 */
export function isValidUtf8(content) {
    try {

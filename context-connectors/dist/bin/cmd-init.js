...
function generateWorkflow(owner, repo, branch, indexKey) {
...
async function runInit(options) {
    ...
    if (!gitInfo) {
        console.error("❌ Could not detect GitHub repository. Make sure you're in a git repo with a GitHub remote.");
        process.exit(1);
    }
    ...
    try {
        ...
        if (!options.force) {
            console.error(`❌ Workflow already exists at ${workflowPath}\n   Use --force to overwrite.`);
            process.exit(1);
        }
    }
    catch {
    ...
    console.log(colorize("green", "The workflow will automatically run on pushes to the " + branch + " branch!"));
}
export const initCommand = new Command("init")
    ...
    .action(runInit);
//# sourceMappingURL=cmd-init.js.map